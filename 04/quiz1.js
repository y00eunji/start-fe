    ...
    var page = 1;
    var see = 3;
    var totpage = Math.ceil(todayPhoto.length/see);
...
function  printimg(page){
    ...
    for (var i = startindex; i < startindex + see; i++) {
        if(todayPhoto[i]){
            str += '<span class="wrapper"><img src="' + todayPhoto[i].img + '"> ' + '<span>' + todayPhoto[i].title + '</span></span>';
        }
    }

    $wrap.innerHTML = str;
    $page.innerHTML = page;
    $totpage.innerHTML = totpage;
}
    printimg(page);
